...
parser.add_argument(
    ...
    default="np",
    type=str,
    help=("Whether to load np/img dataset for folder1"),
)
parser.add_argument(
    "--mode2",
    default="np",
    type=str,
    help=("Whether to load np/img dataset for folder2"),
)
parser.add_argument(
    "path",
    type=str,
...
class NpPathDataset(torch.utils.data.Dataset):
    def __init__(self, files, transforms=None):
        self.files = files
        self.transforms = transforms
...
def get_activations(
    files, model, batch_size=50, dims=2048, device="cpu", num_workers=8, mode="np"
):
...
def calculate_activation_statistics(
    ...
    """
    act = get_activations(
        files, model, batch_size, dims, device, num_workers, mode=mode
    )
...
def compute_statistics_of_path(
    path, model, batch_size, dims, device, num_workers=8, mode="np"
):
...
def calculate_fid_given_paths(
    paths, batch_size, device, dims, num_workers=8, mode1="np", mode2="np"
):
    ...
    m1, s1 = compute_statistics_of_path(
        paths[0], model, batch_size, dims, device, num_workers, mode=mode1
    )
    m2, s2 = compute_statistics_of_path(
        paths[1], model, batch_size, dims, device, num_workers, mode=mode2
    )
